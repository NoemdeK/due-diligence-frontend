...
const Home: React.FC = () => {
  ...
  const { data: querypublish, isLoading } = useQuery(["execHome", currentPage, debouncedSearchValue], () =>
    getExecDrafts(currentPage, "published", debouncedSearchValue),
  );

  useEffect(() => {
    if (querypublish?.data?.result && Object.keys(querypublish?.data?.result).length > 0) {
      setNewData(querypublish?.data?.result);
      setDataCount(querypublish?.data?.count);
    }
  }, [querypublish?.data?.count, querypublish?.data?.result]);

  return (
    <Wrapper className="bg-white rounded-xl">
      <div className="flex items-center justify-between">
        <p className="text-2xl font-[700]">Executives</p>
